...
#pacman -Syy
sudo sfdisk -l 
ask 'Choice volume for EFI: write full path, /dev/sda or /dev/sdc' __EFI_FOLDER__
ask 'Choice volume for ROOT: write full path, /dev/sda or /dev/sdc' __ROOT_FOLDER__

log "Partitioning disk"
cat << HERE | sudo sfdisk "${__EFI_FOLDER__}"
label: gpt
device: "${__EFI_FOLDER__}"

"${__EFI_FOLDER__}": size=500MiB, type=uefi
"${__ROOT_FOLDER__}": type=linux
HERE

echo ''
ask 'LABEL volume for EFI:' __EFI_FOLDER_LABEL__
log "LABEL /boot"
sudo mkfs.fat -F32 -n efi_linux "${__EFI_FOLDER_LABEL__}"

ask 'LABEL volume for ROOT:' __ROOT_FOLDER_LABEL__
log "LABEL /root"
sudo mkfs.ext4 "${__ROOT_FOLDER_LABEL__}" -L new_root
