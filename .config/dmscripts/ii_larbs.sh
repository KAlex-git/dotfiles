...
function log {
  local LINE="$RED[arch-linux-install]$NC$BLUE $* $NC"
  echo -e "$LINE"
  echo -e "${LINE//?/-}"
}
# dotfilesrepo="https://github.com/KAlex-git/dotfiles.git"
# progsfile="https://raw.githubusercontent.com/LukeSmithxyz/LARBS/master/static/progs.csv"
progsfile="/home/kalex/Develop/progs.csv"
aurhelper="yay"
repobranch="master"
export TERM=ansi
...
function check_root() {
    ...
    if [[ ! "${UID}" -eq "${ROOT_UID}" ]]; then
        # Error message
        echo -e  "${RED}Run me as root.${NC}"
        echo -e "${PURPLE}try sudo ./install.sh${NC}"
        exit 1
    fi
}
...
welcomemsg() {
	printf "${PURPLE}
	##############################################
	 # Install add other dependencies program!! #
	#############################################
	${NC}
	"
}
...
aurinstall() {
	log "Installing \`$1\` ($n of $total) from the AUR. $1 $2"
	echo "$aurinstalled" | grep -q "^$1$" && return 1
	sudo -u "$name" $aurhelper -S --noconfirm "$1" >/dev/null 2>&1
}

pipinstall() {
	log "LARBS Installation" 
	echo -e "${RED}Installing the Python package \`$1\` ($n of $total). $1 $2${NC}"
	[ -x "$(command -v "pip")" ] || installpkg python-pip >/dev/null 2>&1
	yes | pip install "$1"
}

installationloop() {
	([ -f "$progsfile" ] && sudo cp "$progsfile" /tmp/progs.csv) ||
	...
	while IFS=, read -r tag program comment; do
		n=$((n + 1))
		echo "$comment" | grep -q "^\".*\"$" &&
			comment="$(echo "${YELLOW}$comment${NC}" | sed -E "s/(^\"|\"$)//g")"
		case "$tag" in
		"A") aurinstall "$program" "$comment" ;;
		"G") gitmakeinstall "$program" "$comment" ;;
		"P") pipinstall "$program" ${PURPLE}"$comment"${NC} ;;
...
#
# echo "
# Setting browser privacy settings and add-ons...
# " 
...
finalize
mediainfo odt2txt highlight atool catdoc docx2txt
